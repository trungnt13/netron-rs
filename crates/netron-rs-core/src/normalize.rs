...
use crate::{
    AttributeValue, Dimension, DimensionValue, Function, FunctionNode, FunctionValue, Graph,
    GraphId, Model, Node, Tensor, TensorElementType, TensorStorage, TypeInfo, Value,
};
...
fn normalize_attribute_value<'a>(
    ...
    match value {
        ...
        AttributeValue::String(value) => {
            NormalizedAttributeValue::String(model.strings.get(*value))
        }
        ...
        AttributeValue::Strings(value) => NormalizedAttributeValue::Strings(
            value.iter().map(|id| model.strings.get(*id)).collect(),
        ),
    ...
    NormalizedTensor {
        ...
        storage: match &tensor.storage {
            ...
            TensorStorage::ElementList { len } => {
                NormalizedTensorStorage::ElementList { len: *len }
            }
