...
impl ServiceState {
    ...
    fn handle(&mut self, request: &ServiceRequest) -> Result<Value, CliFailure> {
        match request.method.as_str() {
            ...
            "summary" => Ok(serde_json::to_value(
                self.session(&request.params)?
                    .summary(&limits_from_params(&request.params)?),
            )
            .map_err(CliFailure::from)?),
            "diagnostics" => Ok(serde_json::to_value(
                self.session(&request.params)?
    ...
    fn open(&mut self, params: &Value) -> Result<Value, CliFailure> {
        ...
        let session =
            ModelSession::open(mapped.bytes(), mapped.source()).map_err(CliFailure::from)?;
    ...
    fn onnx_tensor(&self, params: &Value) -> Result<Value, CliFailure> {
        ...
        let metadata = self
            ...
            .into_iter()
            .find(
                |entry| matches!(entry.handle, EntityHandle::Tensor { tensor: id } if id == tensor),
            )
