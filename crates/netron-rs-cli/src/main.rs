...
fn parse_bench(mut args: Vec<String>) -> Result<Command, CliFailure> {
  ...
  let iterations = match args.len() {
    ...
    _ => {
      return Err(CliFailure::invalid("bench requires <model> [iterations]").with_command("bench"));
    }
  };
  Ok(Command::Bench {
    path: PathBuf::from(args.remove(0)),
...
fn parse_layout(mut args: Vec<String>) -> Result<Command, CliFailure> {
  ...
  let selector = if let Some(node) = node {
    if args.len() != 1 {
      return Err(CliFailure::invalid("layout --node requires <model>").with_command("layout"));
    }
    LayoutSelector::Node {
    ...
    let (path, selected_graph, positional_max) = match args.len() {
      ...
      _ => {
        return Err(
          CliFailure::invalid("layout requires <model> [graph] [max_nodes]").with_command("layout"),
        );
      }
    };
...
fn parse_search(mut args: Vec<String>) -> Result<Command, CliFailure> {
  ...
  let limit = match (flagged_limit, args.len()) {
    ...
    _ => {
      return Err(
        CliFailure::invalid("search requires <model> <query> [limit]").with_command("search"),
      );
    }
  };
  Ok(Command::Search {
...
fn parse_mlir(mut args: Vec<String>) -> Result<Command, CliFailure> {
  ...
  let json = take_flag(&mut args, "--json");
  if args.len() != 1 {
    return Err(CliFailure::invalid("mlir symbols requires <model>").with_command("mlir.symbols"));
  }
