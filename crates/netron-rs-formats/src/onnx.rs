...
fn lower_graph(
    ...
    for mut proto_node in proto.nodes {
        ...
        node.inputs = proto_node
            ...
            .iter()
            .filter_map(|name| {
                ensure_optional_value(model, &mut graph, &mut values, name).map(Some)
            })
            .collect();
...
fn function_input_type_specializations(
    ...
    for (key, uses) in function_uses {
        ...
        for (input_index, function_input) in function.inputs.iter().enumerate() {
            ...
            let Some(call_input) = node.inputs.get(input_index).filter(|name| !name.is_empty())
            else {
                continue;
            };
    ...
    fn decode(data: &[u8]) -> Result<Self, ModelError> {
        ...
        while let Some(field) = reader.next_field()? {
            match field.number {
                ...
                5 => {
                    attribute.kind =
                        Some(AttributeKind::Tensor(TensorProto::decode(field.bytes()?)?))
                }
                6 => {
                    attribute.kind = Some(AttributeKind::Graph(GraphProto::decode(field.bytes()?)?))
                }
